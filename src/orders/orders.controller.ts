...
export class OrdersController {
  ...
  private calculateOrder(products: Product[], articles: Articles[]): number {
    return articles.reduce((sum: number, article: Articles) => {
      const product: Product = products.find(
        (p) => p.id === article.product_id.toString(),
      );
  ...
  private calculateDiscounts(
    ...
    const coffeeProducts = products.filter(
      (product) => product.category === ProductCategoriesEnum.coffee,
    );
    const accessoriesProducts = products.filter(
      (product) => product.category === ProductCategoriesEnum.accesories,
    );
    ...
    const accessoriesTotal = accessoriesProducts.reduce(
      (sum: number, product: Product) => {
        const article: Articles = articles.find(
          (cartArtitle) => cartArtitle.product_id.toString() === product.id,
        );
        return sum + (product.price + article.qty);
      },
      0,
  ...
  private calculateShipping(products: Product[], articles: Articles[]): number {
    const totalOrderProducts: number = articles.reduce(
      (sum: number, article: Articles) => sum + article.qty,
      0,
    );
    const equipmentProducts = products.filter(
      (product) => product.category === ProductCategoriesEnum.equipment,
    );

    // If you buy more than 3 products from the Equipment category, then you get free shipping
    if (equipmentProducts.length > 3) {
      const totalProductEquipments: number = equipmentProducts.reduce(
        (sum: number, product: Product) => {
          const article = articles.find(
            (p) => product.id === p.product_id.toString(),
          );
